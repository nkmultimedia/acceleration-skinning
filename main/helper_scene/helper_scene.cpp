...
GLFWwindow* create_window(const std::string& window_title)
{
    ...
    const int window_width  = int(1280*0.9f);
    const int window_height = int(1000*0.9f);
...
void load_shaders(std::map<std::string,GLuint>& shaders)
{
    ...
    shaders["groundplane"] = create_shader_program("scenes/shared_assets/shaders/mesh/shader.vert.glsl", "scenes/shared_assets/shaders/mesh/shader.frag.glsl");
...
void setup_scene(scene_structure &scene, gui_structure& gui, const std::map<std::string,GLuint>& shaders)
{
    ...
    const image_raw green{ 1,1,image_color_type::rgba,{0,255,0,255} };
    scene.texture_green = create_texture_gpu(green);
}
...
void gui_start_basic_structure(gui_structure& gui, scene_structure& scene)
{
    ...
    if(ImGui::CollapsingHeader("Visual frames"))
    {
