...
function load3DObjects(sceneGraph) {
    ...
    gui.add(sceneElements.param, 'Activated').name("Deformer Active");
    gui.add(sceneElements.param, 'DeformOnly').name("Deformer Only");

    let obj = { Select:function(){viewAllScene()} };
    gui.add(obj,'Select').name( 'View All Scene');
    ...
    loadAsset("cow", [-2,0.75,2], 1.0, R_cow);
    ...
    loadAsset("dragon", [2.0,1.05,-0.5], 0.8, R_dragon);
    ...
    loadAsset("giraffe", [1,0,3], 1.3, Id);
    ...
    loadAsset("sphere", [-0.5,0.35,0], 1.6, R_sphere);
    ...
    gui.add(sceneElements.param, "Wireframe").listen().onChange(changeWireframe);

    gui.add(sceneElements.param, "displaySkeleton").name("Display Skeleton").listen().onChange(changeDisplaySkeleton);
